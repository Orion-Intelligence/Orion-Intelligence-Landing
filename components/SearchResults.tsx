...
import { 
  ...
  Radar, Activity, Lock, ExternalLink
} from 'lucide-react';
...
const SearchResults: React.FC<SearchResultsProps> = ({ query, data, onBack, onNavigateToRemediation }) => {
  ...
  const rawLogUrl = `https://try.orionintelligence.org/api/search/stealerlogs?q=${encodeURIComponent(query)}`;
            ...
            <div className="relative z-10 flex flex-col items-center gap-8">
              ...
              <div className="pt-2">
                <a 
                  href={rawLogUrl} 
                  target="_blank" 
                  className="px-7 py-3 rounded-xl bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-lg flex items-center gap-3"
                >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200 dark:border-white/5">
          ...
          <div className="flex gap-3">
            <a 
              href={rawLogUrl} 
              target="_blank" 
              className="px-6 py-3 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-black text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all flex items-center gap-2 shadow-xl"
            >
